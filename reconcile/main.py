...
def build_pivot_flex(
    ...
    *,
    ...
    req_cols = set(list(index_by) + [stack_col, value_col, "product_lv1", "product_lv2"])
    missing = [c for c in req_cols if c not in df.columns]
    if missing:
        print(f"[skip] {lv1}/{lv2} -> missing columns {missing}; value_col='{value_col}'.")
        return pd.DataFrame()
    ...
    for key, val in (filters or {}).items():
        if key.endswith("_min"):
            ...
            if base not in sub.columns: 
                print(f"[skip-filter] '{base}' not in df; ignoring '{key}'."); continue
            ...
            if key not in sub.columns:
                print(f"[skip-filter] '{key}' not in df; ignoring include filter."); continue
            vals = val if isinstance(val, (list, tuple, set)) else [val]
        ...
        "batches": [
            ...
            },
            {
                ...
                "charts": [
                    ...
                    {"lv1": "vehicle", "lv2": "electric",     "title": "Capacity -Electric vehicles by company and status",    "filename": "capacity_electric_vehicles_company_status.png"}
                ]
            },
            {
