...
from src.config import PRODUCT_CLASSIFICATION
...
if not logger.hasHandlers():
    ...
    # Log file for product updater
    log_path = f"logs/product_update_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    file_handler = logging.FileHandler(log_path)
    file_handler.setLevel(logging.INFO)

    # Optional: only log WARNING+ to console (or use StreamHandler if needed)
